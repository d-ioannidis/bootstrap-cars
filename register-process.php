<?php

  $register_success = <<<TEXT
  <div style="max-width:1100px;margin-top:80px;margin-bottom:80px">
    <h2>Your registration has been successful.</h2>
  </div>
  TEXT;

  $existerror = <<<TXT
  <div style="max-width:1100px;margin-top:80px;margin-bottom:80px">  
    <h2>Registration failed, either the username or email is already in use.</h2>  
  </div>
  TXT;

  $emailerror = <<<TXT
  <div style="max-width:1100px;margin-top:80px;margin-bottom:80px">  
    <h2>Email address given is invalid or doesn't exist.</h2>  
  </div>
  TXT;

  $passworderror = <<<TXT
  <div style="max-width:1100px;margin-top:80px;margin-bottom:80px">  
    <h2>Invalid password. Make sure your password complies with the requirements shown below:</h2>
    <ul>
      <li>Must include at least 8 characters.</li>
      <li>At least 1 lowercase character.</li>
      <li>At least 1 uppercase character.</li>
      <li>At least 1 numeric character.</li>
      <li>At least 1 special character.</li>
      <li>Make sure both password fields are equal.</li>
    </ul>  
  </div>
  TXT;

  $firstname = $_POST['firstname'];
  $lastname = $_POST['lastname'];
  $email = $_POST['email'];
  $username = $_POST['username'];
  $password = $_POST['passwordfield'];
  $cpassword = $_POST['cpasswordfield'];

  $conn = new mysqli("localhost", "root","", "electric_cars");

  $sql_user = "SELECT * FROM user WHERE username='$username'";
  $sql_email = "SELECT * FROM user WHERE email='$email'";
  $res_user = $conn->query($sql_user);
  $res_email = $conn->query($sql_email);

  if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo $emailerror;
    header("refresh:5;url=register.php");
  } else if (mb_strlen($password) <= 8 
  || !preg_match("#[0-9]+#",$password) 
  || !preg_match("#[A-Z]+#",$password) 
  || !preg_match("#[a-z]+#",$password) 
  || !preg_match("#[\W]+#", $password)
  || $password != $cpassword) 
  {
    echo $passworderror;
    header("refresh:5;url=register.php");
  } else if (mysqli_num_rows($res_user) > 0 || mysqli_num_rows($res_email) > 0)
    {
      echo $existerror;
      header("refresh:5;url=register.php");
    } else {
        echo $register_success;
        $insert_sql = "INSERT INTO user (firstname, lastname, username, `password`, email, `role`)
        VALUES ('$firstname', '$lastname', '$username', '$password', '$email', 'Member')";
      
        if ($conn->query($insert_sql) === TRUE) {
          //echo "New record created successfully";
        } else {
          //echo "Error: " . $sql . "<br>" . $conn->error;
        }
        header("refresh:5;url=login.php");
    }

    /* close connection */
    $conn->close();
  ?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link href="css/bootstrap.min.css" rel="stylesheet">
    <link href="css/bootstrap-icons.css" rel="stylesheet">
    <link rel="stylesheet" href="css/global.css" />
    <title>Electric Cars</title>

    <style>
        .footer {
            position: fixed;
            left: 0;
            bottom: 0;
            width: 100%;            
            text-align: center;
        }
    </style>
  </head>
  <body>
    <!-- Navbar -->
    <?php 
      include('navbar.php');
    ?>

    <!-- Footer -->
    <?php 
      include('footer.php');
    ?>        

    <script src="js/bootstrap.bundle.min.js"></script>    
    
  </body>
</html>
